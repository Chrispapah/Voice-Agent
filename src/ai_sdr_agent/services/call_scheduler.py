        ...
        if self.settings is not None and not self._bot_config:
            ...
            transcriber_config = DeepgramTranscriberConfig.from_telephone_input_device(
                endpointing_config=DeepgramEndpointingConfig(
                    vad_threshold_ms=self._cfg("deepgram_vad_threshold_ms", 55),
                    utterance_cutoff_ms=self._cfg("deepgram_utterance_cutoff_ms", 1500),
                    time_silent_config=TimeSilentConfig(
                        time_cutoff_seconds=max(
                            self._cfg("deepgram_time_cutoff_seconds", 0.05), 0.04
                        ),
                        post_punctuation_time_seconds=max(
                            self._cfg("deepgram_post_punctuation_time_seconds", 0.02), 0.02
                        ),
                    ),
